...
def apply_script(protocol, connection, config):
    ...
    class TowerOfBabelConnection(connection):
        def get_protected_area(self, team):
            # Petite zone de protection autour de la base
            if team is self.protocol.blue_team:
                return 256 - 138, 256 - 138 + 10, 256 - 10, 256 + 10
            else:
                return 256 + 138 - 10, 256 + 138, 256 - 10, 256 + 10
            # if team is self.protocol.blue_team:
        ...
        def on_block_destroy(self, x, y, z, mode):
            ...
            if (not self.is_trusted_for_block_destruction() and
                ...
                if minx <= position.x <= maxx and miny <= position.y <= maxy:
                    self.send_chat(
                        'You can\'t destroy enemy team\'s '
            ...
            # Calcul distance entre le joueur et le tir
            distance = sqrt((position.x - x)**2 + (position.y - y)**2)
            # Si le joueur est trop loin, on ne peut pas casser le bloc
            can_shoot_blocks = distance >= 55

            if can_shoot_blocks:
                if self.tool is WEAPON_TOOL:
                    self.send_chat('Your weapon is not enough powerful to destroy blocks at this distance. Try to get closer!')
                elif self.tool is GRENADE_TOOL:
                    self.send_chat(
